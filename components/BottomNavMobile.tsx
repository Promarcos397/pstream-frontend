        ...
        {items.map((item, idx) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center gap-0.5 rounded-full px-6 sm:px-7 py-1.5 select-none
                transition-colors duration-200 active:scale-95
                ${isActive ? 'bg-white/[0.12] text-white' : 'text-white/55 hover:text-white/80'}`}
            >
              <span className="flex items-center justify-center h-[22px]">{item.icon}</span>
