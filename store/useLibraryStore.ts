  ...
  persist(
    (set, get) => ({
      ...
      setRating: (tmdbId, type, rating, movieData) => {
        ...
        const newRating: UserRating = { tmdbId, type, rating, movieData, updatedAt };
      ...
      getRating: (tmdbId) => {
        return get().ratings[String(tmdbId)]?.rating;
      },
      ...
      toggleMyList: (movie) => {
        ...
        set((state) => {
          ...
          if (isPresent) {
            ...
            next[key] = {
              tmdbId: key,
              type: movie.name ? 'tv' : 'movie', // Fallback type checking
              movieData: movie,
              addedAt: Date.now()
            };
          }
      ...
      isInList: (tmdbId) => {
        return !!get().myList[String(tmdbId)];
      },
      ...
      syncFromCloud: (cloudRatings, cloudList) => {
        set((state) => {
          ...
          cloudRatings.forEach(cr => {
            ...
            if (!local || cr.updatedAt > local.updatedAt) {
              mergedRatings[cr.tmdbId] = cr;
            }
          });
          ...
          cloudList.forEach(cl => {
            ...
            if (!local || cl.addedAt > local.addedAt) {
              mergedList[cl.tmdbId] = cl;
            }
          });
